...
nohup ros2 run robot_state_publisher robot_state_publisher "${LEFT_URDF_FILE}" --ros-args -r __node:=left_robot_state_publisher >/tmp/rsp_left_dual_arm.log 2>&1 </dev/null &
nohup ros2 run robot_state_publisher robot_state_publisher "${RIGHT_URDF_FILE}" --ros-args -r __node:=right_robot_state_publisher >/tmp/rsp_right_dual_arm.log 2>&1 </dev/null &

# Spawn arms near the left/right sides of the table.
timeout 20 ros2 run ros_gz_sim create -world "${WORLD_NAME}" -name left_ur5e_hande -file "${LEFT_URDF_FILE}" -x 0.0 -y 0.60 -z 0.0 -Y -1.5708
timeout 20 ros2 run ros_gz_sim create -world "${WORLD_NAME}" -name right_ur5e_hande -file "${RIGHT_URDF_FILE}" -x 0.0 -y -0.60 -z 0.0 -Y 1.5708
